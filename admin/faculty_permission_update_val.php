        ...
        if(isset($_POST['submit']))
        {
           ...
           /* $qry = "select batch_id from batch where batch_id='$batch_id';";
            ...
            {*/
                ...
                $qry = "select * from faculty_permission where sub_id='$sub_id' AND f_id='$f_id' AND fac_per_id!='$fac_per_id';";
                $result = mysqli_query($con,$qry);
                $row = mysqli_num_rows($result);
                if($row>0)
                    {
                         echo "<script>alert('Subject allready assign in This Faculty.')</script>";
                        echo '<script>window.location="../admin/faculty_permission.php"</script>';
                    }
                    else
                    {
                        $qry2 = "UPDATE `faculty_permission` SET `f_id`='$f_id',`sub_id`='$sub_id' where fac_per_id='$fac_per_id';";
